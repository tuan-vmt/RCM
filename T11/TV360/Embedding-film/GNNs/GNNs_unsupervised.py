...
class GNN(object):
    ...
    """
    def __init__(self, adj_matrix, config, features=None):
        super(GNN, self).__init__()
        '''
        default params:
        features=None, labels=None, supervised=False, model='gat', n_layer=2, emb_size=128, 
        random_state=1234, device='auto', epochs=5, batch_size=20, lr=0.7, unsup_loss_type='margin', print_progress=True
        '''
        # fix random seeds
        ...
        self.unsup_loss_type = config.unsup_loss_type
        ...
        # load data
        self.dl = DataLoader(adj_matrix, features, config.labels, self.supervised, self.device)

        self.gnn = GNN_model(self.n_layer, self.emb_size, self.dl, self.device, gat=self.gat, gcn=self.gcn)
